...
/*   Updated: 2024/01/17 16:04:56 by ngriveau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    if (!this->getSigned()) {
        std::cout << executor.getName() << " not execute " << this->getName() << "Beacause form not sign";
        throw AForm::GradeTooLowException("Form not sign");
    }
    else if (this->getGradeForExecute() <= executor.getGrade())
    {
        std::cout << executor.getName() << " not execute " << this->getName() << "Beacause bureaucrat not have a grade for execute";
